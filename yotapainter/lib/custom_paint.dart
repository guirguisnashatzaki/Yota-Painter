...
class _CustomPaintPageState extends State<CustomPaintPage> {
  ...
  int gettappedshape(Offset tappedPoint){
    int index=0;
    double min = double.maxFinite;
    for(int i=0;i<customPaintNotifier.points.length;i++){
      if(min>_calcdistancebetshapeandpoint(tappedPoint, customPaintNotifier.points[i])){
        min=_calcdistancebetshapeandpoint(tappedPoint, customPaintNotifier.points[i]);
        index=i;
      }
    }
    return index;
  ...
  void deletetappedshape(Offset tappedPoint){
    customPaintNotifier.remove(gettappedshape(tappedPoint));
  }
  ...
  bool compareshapes(DrawingArea first,DrawingArea second){
    if(first.mainPoint.dx==second.mainPoint.dx && first.mainPoint.dy==second.mainPoint.dy){
      if(first.secondaryPoint.dx==second.secondaryPoint.dx && first.secondaryPoint.dy==second.secondaryPoint.dy){
  ...
  bool searchshape(DrawingArea shape){

    for(int i=0;i<customPaintNotifier.points.length;i++){
      if(compareshapes(shape, customPaintNotifier.points[i])){
        return true;
      }
    }
  ...
  Future<bool> takePicture() async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (builder)=> MyColouringPage(pngBytes: pngBytes,list: customPaintNotifier.points,x:widget.numberofsquares_x ,y:widget.numberofsquares_y ,name: widget.name,)));
    return false;
  }
  ...
  Future<bool> takePictureExport() async {
    ...
    ByteData? byteData = await image.toByteData(format: ui.ImageByteFormat.png);
    Uint8List pngBytes = byteData!.buffer.asUint8List();
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (builder)=> Saving_Page(pngBytes: pngBytes,list: customPaintNotifier.points,x:widget.numberofsquares_x ,y:widget.numberofsquares_y ,name: widget.name,)));
    return false;
  }
                        ...
                        if(selectingPointsNotifier.selectingPoints==true){
                          if(colouringNotifier.colouring){
                            ...
                            if(controlDrawing==true){
                              ...
                              if(!compareoffsets(checkedOffset, temp)){
                                DrawingArea shape=DrawingArea(
                                ...
                                if(!searchshape(shape)){
                                 customPaintNotifier.addPoint(shape);
                                }
                              }
